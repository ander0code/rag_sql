# Agente clarificador: busca opciones en la DB cuando hay ambigüedad
...
# Busca opciones de clarificación consultando la DB
class ClarifyAgent:
    ...
    def get_options(self, entity_type: str) -> List[str]:
        ...
        return self._fetch_options(schema_name, table_name, display_field)

    # Encuentra dinámicamente el mejor campo para mostrar opciones
    ...
    # Ejecuta query para obtener opciones únicas de la tabla
    def _fetch_options(
        self, schema_name: str, table_name: str, field: str
    ) -> List[str]:
        query = f'''
        ...
        try:
            ...
            if "error" not in result and result.get("data"):
                ...
                logger.debug(
                    f"Opciones de {schema_name}.{table_name}.{field}: {len(options)}"
                )
                return options
